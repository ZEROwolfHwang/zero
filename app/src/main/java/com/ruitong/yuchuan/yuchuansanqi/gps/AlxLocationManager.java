...
public class AlxLocationManager implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
    ...
    public static void onCreateGPS(final Application context) {
        ...
        manager.context = sContext;
        manager.mGoogleApiClient = new GoogleApiClient.Builder(sContext)
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                if (manager == null || manager.context == null || manager.currentStatus != STATUS.NOT_CONNECT)
                    return;//如果连接GPS硬件到9s后还没成功
                sContext.startService(new Intent(manager.context, AlxLocationService.class));
    ...
    public boolean getCurrentLocation() {
        ...
        if (last_accuracy < MAX_deviation) {
            recordLocation(context, latitude, longitude, last_accuracy, speed);
        } else {
    ...
    public void onLocationChanged(Location location) {
        ...
        if (location.getAccuracy() < MAX_deviation) {//精度如果太小就放弃
            recordLocation(context, location.getLatitude(), location.getLongitude(), location.getAccuracy(), location.getSpeed());
        } else {
     ...
     * @param speed
     */
    public static void recordLocation(Context context, double latitude, double longitude, float accuracy, float speed){
...
//            if(isDebugging)
//                Toast.makeText(context,"最终记录的经过火星转换的结果是latitude="+latitude+"   longitude="+longitude+"   accuracy="+accuracy, Toast.LENGTH_LONG).show();
        }else {
